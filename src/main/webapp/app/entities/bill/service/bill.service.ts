...
type RestOf<T extends IBill | NewBill> = Omit<T, 'date' | 'createdDate' | 'lastModifiedDate'> & {
  date?: string | null;
...
export class BillService {
  ...
  protected convertDateFromClient<T extends IBill | NewBill | PartialUpdateBill>(bill: T): RestOf<T> {
    return {
      ...bill,
      date: bill.date?.toJSON() ?? null,
      createdDate: bill.createdDate?.toJSON() ?? null,
      lastModifiedDate: bill.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restBill: RestBill): IBill {
    return {
      ...restBill,
      date: restBill.date ? dayjs(restBill.date) : undefined,
