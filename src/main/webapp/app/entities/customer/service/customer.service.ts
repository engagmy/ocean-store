...
type RestOf<T extends ICustomer | NewCustomer> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class CustomerService {
  ...
  protected convertDateFromClient<T extends ICustomer | NewCustomer | PartialUpdateCustomer>(customer: T): RestOf<T> {
    return {
      ...
      lastModifiedDate: customer.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restCustomer: RestCustomer): ICustomer {
    return {
      ...
      lastModifiedDate: restCustomer.lastModifiedDate ? dayjs(restCustomer.lastModifiedDate) : undefined,
    };
  }
