...
type RestOf<T extends IEmployee | NewEmployee> = Omit<T, 'joinDate' | 'createdDate' | 'lastModifiedDate'> & {
...
export class EmployeeService {
  ...
  protected convertDateFromClient<T extends IEmployee | NewEmployee | PartialUpdateEmployee>(employee: T): RestOf<T> {
    return {
      ...
      lastModifiedDate: employee.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restEmployee: RestEmployee): IEmployee {
    return {
      ...
      lastModifiedDate: restEmployee.lastModifiedDate ? dayjs(restEmployee.lastModifiedDate) : undefined,
    };
  }
