...
type RestOf<T extends ISale | NewSale> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class SaleService {
  ...
  protected convertDateFromClient<T extends ISale | NewSale | PartialUpdateSale>(sale: T): RestOf<T> {
    return {
      ...sale,
      createdDate: sale.createdDate?.toJSON() ?? null,
      lastModifiedDate: sale.lastModifiedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restSale: RestSale): ISale {
    return {
      ...restSale,
      createdDate: restSale.createdDate ? dayjs(restSale.createdDate) : undefined,
      lastModifiedDate: restSale.lastModifiedDate ? dayjs(restSale.lastModifiedDate) : undefined,
    };
  }
