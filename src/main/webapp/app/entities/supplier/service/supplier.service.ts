...
type RestOf<T extends ISupplier | NewSupplier> = Omit<T, 'createdDate' | 'lastModifiedDate'> & {
...
export class SupplierService {
  ...
  protected convertDateFromClient<T extends ISupplier | NewSupplier | PartialUpdateSupplier>(supplier: T): RestOf<T> {
    return {
      ...supplier,
      createdDate: supplier.createdDate?.toJSON() ?? null,
  ...
  protected convertDateFromServer(restSupplier: RestSupplier): ISupplier {
    return {
      ...restSupplier,
      createdDate: restSupplier.createdDate ? dayjs(restSupplier.createdDate) : undefined,
